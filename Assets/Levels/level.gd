...
func _ready() -> void:
	...
	}, _on_platform_destroyed, _add_coin)
...
func _on_teleport_zone_body_entered(body: Node3D) -> void:
	...
	if body.name == "Player" and spawn_points.size() > 0:
		var random_spawn = spawn_points[randi() % spawn_points.size()]
		body.global_transform.origin = random_spawn.global_transform.origin + Vector3(0, 0.5, 0)# Replace with function body.
